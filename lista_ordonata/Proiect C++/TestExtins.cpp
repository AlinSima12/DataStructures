...
void testStergeCauta(Relatie r) {
	...
	try {
		...
		it1.prim();
		TElement nouPrim = it1.element(); //aici
...
void testCantitativ(){
    ...
    for (int v = vMin-d; v <= vMax+d; v++){
		it.prim();
		//while (it.valid()) {
		//	cout << it.element() << '-' << lo.cauta(it.element()) << ' ';
		//	it.urmator();
		//}
		//cout << endl;
...
void testUltimPoz(Relatie r) {
	...
	assert(lo.ultimulIndex(2) == 4);
	assert(lo.ultimulIndex(5) == 5);
	assert(lo.ultimulIndex(6) == 6);
	assert(lo.ultimulIndex(10) == 8);
	}
